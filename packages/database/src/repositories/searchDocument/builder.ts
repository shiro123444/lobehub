...
import type {
  ...
  SearchDocumentKey,
  SearchDocumentSourceMap,
} from './schema';
import { parseSearchDocumentSource, SEARCH_DOCUMENT_ENTITIES } from './schema';

interface SearchDocumentSelection {
  afterId?: string;
  ids?: string[];
  limit: number;
}

export interface SearchDocumentBatchOptions {
  afterId?: string;
  limit: number;
}

export type SearchDocumentRelationChange =
  | { fileIds: readonly string[]; relation: 'knowledgeBaseFiles' }
  | { memoryIds: readonly string[]; relation: 'userMemoryReferences' };

const entityOrder = new Map(SEARCH_DOCUMENT_ENTITIES.map((entity, index) => [entity, index]));
...
const compareKeys = (left: SearchDocumentKey, right: SearchDocumentKey) => {
...
const dedupeKeys = (keys: SearchDocumentKey[]) =>
 ...
 * need the complete source-of-truth projection. Product reads must continue to use SearchRepo,
...
export class SearchDocumentBuilder {
  ...
  async buildBatch(
    ...
    { afterId, limit }: SearchDocumentBatchOptions,
  ): Promise<BuiltSearchDocument[]> {
    if (!Number.isInteger(limit) || limit < 1) {
      throw new Error('Search document batch limit must be a positive integer');
    }
  ...
  async resolveAffectedKeys(change: SearchDocumentRelationChange): Promise<SearchDocumentKey[]> {
    if (change.relation === 'knowledgeBaseFiles') {
      ...
      return dedupeKeys([
        ...fileIds.map((id): SearchDocumentKey => ({ entity: 'files', id })),
        ...linkedDocuments.map(({ id }): SearchDocumentKey => ({ entity: 'documents', id })),
      ]);
    }
    ...
    return dedupeKeys([
      ...memoryIds.map((id): SearchDocumentKey => ({ entity: 'userMemories', id })),
      ...contexts.map(({ id }): SearchDocumentKey => ({ entity: 'memoryContexts', id })),
      ...preferences.map(({ id }): SearchDocumentKey => ({ entity: 'memoryPreferences', id })),
      ...activities.map(({ id }): SearchDocumentKey => ({ entity: 'memoryActivities', id })),
      ...identities.map(({ id }): SearchDocumentKey => ({ entity: 'memoryIdentities', id })),
      ...experiences.map(({ id }): SearchDocumentKey => ({ entity: 'memoryExperiences', id })),
    ]);
  }

  private createDocument<Entity extends SearchDocumentEntity>(
    entity: Entity,
    input: unknown,
  ): Extract<BuiltSearchDocument, { entity: Entity }> {
    const source = parseSearchDocumentSource(entity, input);
    return { entity, id: source.id, source } as Extract<BuiltSearchDocument, { entity: Entity }>;
  }

  private async build(
    entity: SearchDocumentEntity,
    selection: SearchDocumentSelection,
  ): Promise<BuiltSearchDocument[]> {
    switch (entity) {
      case 'agents': {
  ...
  private async buildAgents(selection: SearchDocumentSelection) {
  ...
  private async buildTopics(selection: SearchDocumentSelection) {
    const rows = await this.db
      .select({
  ...
  private async buildFiles(selection: SearchDocumentSelection) {
    const rows = await this.db
      .select({
  ...
  private async buildKnowledgeBases(selection: SearchDocumentSelection) {
    const rows = await this.db
      .select({
  ...
  private async buildUserMemories(selection: SearchDocumentSelection) {
    const rows = await this.db
      .select({
  ...
  private async buildChatGroups(selection: SearchDocumentSelection) {
    const rows = await this.db
      .select({
  ...
  private async buildMemoryContexts(selection: SearchDocumentSelection) {
    const rows = await this.db
      .select({
  ...
  private async buildMemoryPreferences(selection: SearchDocumentSelection) {
    const rows = await this.db
      .select({
  ...
  private async buildMemoryActivities(selection: SearchDocumentSelection) {
    const rows = await this.db
      .select({
  ...
  private async buildMemoryIdentities(selection: SearchDocumentSelection) {
    const rows = await this.db
      .select({
  ...
  private async buildMemoryExperiences(selection: SearchDocumentSelection) {
    const rows = await this.db
      .select({
  ...
  private async buildPersonaDocuments(selection: SearchDocumentSelection) {
    const rows = await this.db
      .select({
  ...
  private async buildDocuments(selection: SearchDocumentSelection) {
    const rows = await this.db
      .select({
  ...
  private async buildMessages(selection: SearchDocumentSelection) {
    const rows = await this.db
      .select({
...
export type SearchDocumentFor<Entity extends SearchDocumentEntity> = Extract<
  BuiltSearchDocument,
  { entity: Entity; source: SearchDocumentSourceMap[Entity] }
>;
