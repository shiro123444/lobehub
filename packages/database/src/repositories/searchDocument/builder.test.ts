...
import { SearchDocumentBuilder } from './builder';
import { SEARCH_DOCUMENT_ENTITIES } from './schema';
...
describe('SearchDocumentBuilder', () => {
  it('builds all 14 canonical fixtures with stable IDs and deterministic payloads', async () => {
    for (const entity of SEARCH_DOCUMENT_ENTITIES) {
      const fixture = SEARCH_DOCUMENT_FIXTURES[entity];
  ...
  it('rejects invalid batch limits before querying PostgreSQL', async () => {
    await expect(builder.buildBatch('agents', { limit: 0 })).rejects.toThrow(
      'Search document batch limit must be a positive integer',
    );
  });
