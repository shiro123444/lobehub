...
export const SEARCH_DOCUMENT_SCHEMAS = {
  agents: z
    .object({
      ...ownershipShape,
      ...timestampShape,
  ...
  userMemories: z
    ...
    .strict(),
} as const satisfies Record<SearchDocumentEntity, z.ZodType>;

export type SearchDocumentSourceMap = {
  [Entity in SearchDocumentEntity]: z.infer<(typeof SEARCH_DOCUMENT_SCHEMAS)[Entity]>;
};

export interface SearchDocumentKey<Entity extends SearchDocumentEntity = SearchDocumentEntity> {
  entity: Entity;
  id: string;
}

export type BuiltSearchDocument = {
  [Entity in SearchDocumentEntity]: SearchDocumentKey<Entity> & {
    source: SearchDocumentSourceMap[Entity];
  };
}[SearchDocumentEntity];

export const parseSearchDocumentSource = <Entity extends SearchDocumentEntity>(
  entity: Entity,
  input: unknown,
): SearchDocumentSourceMap[Entity] =>
  SEARCH_DOCUMENT_SCHEMAS[entity].parse(input) as SearchDocumentSourceMap[Entity];
