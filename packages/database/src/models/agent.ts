...
export class AgentModel {
  ...
  updateConfig = async (agentId: string, input: PartialDeep<AgentItem> | undefined | null) => {
     ...
     * (e.g. switching to Claude Code / Codex), and the write races
     * `AgentShareModel.updateVisibility`'s own `FOR UPDATE` lock on the same
     * row. `writeAgentConfigWithShareReset` (packages/database/src/utils/
     * agentConfigShareReset.ts) is the single choke point that locks the row,
