 ...
 *    `is_deleted = false`, so a trashed row is invisible in place. The row —
...
export const trashItems = pgTable(
  ...
  {
    id: text('id')
      .$defaultFn(() => idGenerator('trashItems'))
      .primaryKey(),
     ...
     */
    rootId: text('root_id').references((): any => trashItems.id, { onDelete: 'cascade' }),
