...
- 路由缓冲复用 `BrandTextLoading` / `NexusBrandLoading`。
...
每次进入 `/presentation` 路由使用现有 Nexus loading：

- 普通桌面动态路由和桌面静态配置保持一致。
- 路由层负责 `BrandTextLoading` / `NexusBrandLoading`。
- 页面内部只负责自己的数据 loading，不重复绘制 Nexus。
...
完成条件：1440×900 不窄、不出现中间滚动条；390×844 不溢出；进入路由显示 Nexus loading。
...
- 继续复用 `@lobehub/ui`、现有 `styles`、Nexus/品牌动效；不引入清舟 UI 组件，不添加第二套 loading 体系。
